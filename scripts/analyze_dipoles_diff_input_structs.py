...
def main():
    ...
    for n in range(n_runs):
        ...
        y1, bin_edges = np.histogram(mu_z_1, bins=50)
        kernel_1 = stats.gaussian_kde(mu_z_1)

        y2, bin_edges = np.histogram(mu_z_2, bins=50)
        kernel_2 = stats.gaussian_kde(mu_z_2)
        ...
        p_y_1 = kernel_1.evaluate(bin_centers)
        p_y_2 = kernel_2.evaluate(bin_centers)


        plt.plot(bin_centers, p_y_1, 'k-', color="red")
        plt.plot(bin_centers, p_y_2, 'k-', color="blue")
        ...
        plt.tight_layout()
        plt.savefig("images/%s_polar_vs_apolar_run_%02d" % (chg, n))
